...
class DataProcessor:
    ...
    def filter_user(self, min_rating=400):
        try:
            n_ratings = self.rating_df["user_id"].value_counts()

            ## For choosing experienced user only
    ...
    def encode_data(self):
        try:
            ...
            # Doing encoding so our model can understand (Ex: 10054 --> 98)
            self.user2user_encoded = {x : i for i , x in enumerate(user_ids)}

            # Encoded values are for model only so we decode for user 
    ...
    def save_artifacts(self):
            try:
                ...
                self.rating_df.to_csv(RATING_DF,index=False) ## Index = Flase to ensure there are no unnecessary columsn in the saved csv file
    ...
    def process_anime_data(self):
            try:
                ...
                raise CustomException("Failed to save DF and SYNOPSIS_DFs data",sys)
    ...
    def run(self):
        try:
            ...
            logger.info("Data Processing Pipeline run Sucessfully.. Congrats")
